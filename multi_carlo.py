...
@nb.jit(nopython=True)
def hopping(maxiter,unit_time,num):
...
if __name__=='__main__':
    ...
    unit=1000
    maxtime_iter=100000
    walkers=100 # number of walkers, can change
    num_cores = int(mp.cpu_count())
    pool = mp.Pool(num_cores)

    results = [pool.apply_async(hopping, args=(maxtime_iter,unit,i+1)) for i in range(walkers)]
    results = [p.get() for p in results]

    data=np.zeros(int(maxtime_iter/unit))
    for i in range(len(data)):
        temp=0
        for j in range(len(results)):
            temp+=results[j][i]
        data[i]=temp/len(results)

    plt.scatter(range(unit,maxtime_iter+unit,unit),data)

    A,B=optimize.curve_fit(f,range(unit,maxtime_iter+unit,unit)[10:],data[10:])[0]
    newy=[f(x,A,B) for x in range(20000,maxtime_iter+unit,int(unit/100))]
    # 1s= unit=100
    #plt.xlabel('t',fontsize=13)
    #plt.ylabel(r'$<r^2>$',fontsize=13)
    plt.plot(range(20000,maxtime_iter+unit,int(unit/100)),newy,c='red')
